...
export class CustomHeaderDirective implements AfterViewInit {
  ...
  constructor(
    ...
    private headerRow: ElementRef,
    private dndService: DragDrop
  ) {}

  public ngAfterViewInit(): void {
    this.dropListRef = this.createDropList(
      this.headerRow,
      this.createDrags(this.getHeaderCells(this.headerRow.nativeElement))
    );
    this.columns = this.table._contentHeaderRowDefs.first.columns as Array<
      string
    >;
  ...
  private createDropList(
    headerRowDef: ElementRef,
    drags: Array<DragRef>
  ): DropListRef {
    const dropListRef: DropListRef = this.dndService.createDropList(
      headerRowDef
    );
    dropListRef.withItems(drags);
    dropListRef.withOrientation('horizontal');
    dropListRef.dropped.subscribe(
      (event: {
        item: DragRef;
        currentIndex: number;
